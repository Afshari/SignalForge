...
{
    uint32_t fft_size = 65536;
    auto mags = FFTWavFile(TestDataPath("engine_clean_500kb_001.wav"), fft_size);
    EXPECT_EQ(mags.size(), fft_size / 2 + 1);
}

TEST(FFTTests, AllMagnitudes_AreNonNegative)
{
    auto mags = FFTWavFile(TestDataPath("engine_clean_500kb_001.wav"));
...
{
    auto mags_a = FFTWavFile(TestDataPath("engine_clean_500kb_001.wav"));
    auto mags_b = FFTWavFile(TestDataPath("engine_clean_500kb_001.wav"));
...
{
    auto mags_a = FFTWavFile(TestDataPath("engine_clean_500kb_001.wav"));
    auto mags_b = FFTWavFile(TestDataPath("engine_clean_500kb_002.wav"));
    ASSERT_EQ(mags_a.size(), mags_b.size());
...
{
    auto mags_clean = FFTWavFile(TestDataPath("engine_clean_500kb_001.wav"));
    auto mags_noisy = FFTWavFile(TestDataPath("engine_noisy_500kb_001.wav"));
...
{
    auto mags_a = FFTWavFile(TestDataPath("engine_noisy_500kb_001.wav"));
    auto mags_b = FFTWavFile(TestDataPath("engine_noisy_500kb_002.wav"));
    ASSERT_EQ(mags_a.size(), mags_b.size());
...
TEST(FFTTests, AnomalySignal_HasStrongerPeakAt1350Hz_ThanClean)
{
    ...
    auto mags_anomaly = FFTWavFile(TestDataPath("engine_anomaly_500kb_001.wav"), fft_size);
    auto mags_clean = FFTWavFile(TestDataPath("engine_clean_500kb_001.wav"), fft_size);
...
TEST(FFTTests, BatchFFT_MatchesSingleFFT)
{
    ...
    SignalForge::WavReader reader_b(TestDataPath("engine_clean_500kb_002.wav"));
