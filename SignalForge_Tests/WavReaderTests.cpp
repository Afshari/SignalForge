...
{
    EXPECT_THROW(
        SignalForge::WavReader reader(TestDataPath("nonexistent.wav")),
        std::runtime_error
    );
}
...
{
    auto path = TestDataPath("invalid_riff.wav");
...
{
    auto path = TestDataPath("non_pcm.wav");
    std::filesystem::create_directories(path.parent_path());
...
{
    auto path = TestDataPath("test_samplerate.wav");
    std::filesystem::create_directories(path.parent_path());

    std::vector<int16_t> samples(1024, 0);
    WriteMinimalWav(path, samples, 44100);
...
{
    auto path = TestDataPath("test_channels.wav");
    std::filesystem::create_directories(path.parent_path());

    std::vector<int16_t> samples(1024, 0);
    WriteMinimalWav(path, samples, 44100, 1);
...
{
    auto path = TestDataPath("test_bitdepth.wav");
    std::filesystem::create_directories(path.parent_path());

    std::vector<int16_t> samples(1024, 0);
    WriteMinimalWav(path, samples);
...
{
    auto path = TestDataPath("test_numsamples.wav");
    std::filesystem::create_directories(path.parent_path());
...
{
    auto path = TestDataPath("test_pcm_size.wav");
    std::filesystem::create_directories(path.parent_path());
...
{
    auto path = TestDataPath("test_pcm_values.wav");
    std::filesystem::create_directories(path.parent_path());
...
{
    auto path = TestDataPath("test_pcm_twice.wav");
    std::filesystem::create_directories(path.parent_path());
